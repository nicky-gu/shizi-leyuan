...
> 一年级升二年级 · 20 天攻克二年级上册 607 个生字

一个为小朋友设计的识字网站：**每日特训 + 艾宾浩斯记忆曲线巩固 + 跨终端同步**，纯前端 + Cloudflare 免费服务，零成本运行。
...
| 模式 | 说明 |
|---|---|
| 🪪 识别卡 | 大字卡片 + 拼音 + 点读发音（语音合成） |
| 🔊 听令选字 | 听读音，从 4 个字中锁定听到的字 |
| 🔤 看字选读音 | 看汉字选正确读音 |
| 🎯 目标锁定 | 在 12 个字中快速锁定目标字 |

### 🔄 每日战术复习模块（艾宾浩斯记忆曲线）
- 学过的字在第 **1、2、4、7、15** 天自动到期巩固
- **未掌握优先**排在复习最前面，另有「未掌握专项加练」随时加练
- 自动制定每日任务（新训 + 巩固）

### ☁️ 跨终端同步（Cloudflare Workers KV）
- 电脑 / iPad / 手机共享同一份进度
- **同步码即凭证**（用户自己起一个好记的（如 `XIAOMING-2019`）），无需注册、无需 token
